...
class _ClearPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double fade = math.pow(1 - progress, 2).toDouble();
    final Paint flash = Paint()
      ..color = const Color(0xFFFFE6A3).withValues(alpha: fade * .85);
    ...
    for (int row = 0; row < GameEngine.size; row++) {
      for (int col = 0; col < GameEngine.size; col++) {
        ...
        final double side = cell * (1 - progress * .7);
        canvas.drawRRect(RRect.fromRectAndRadius(
          Rect.fromCenter(center: center, width: side, height: side),
          const Radius.circular(3),
        ), flash);
        // Four deterministic particles per affected cell, including a row/col
        // intersection only once. No timers or random allocations per frame.
        for (int i = 0; i < 4; i++) {
          final double angle = i * math.pi / 2 + (row + col) * .7;
          ...
          spark.color = (i.isEven ? Colors.white : const Color(0xFFFFCE58))
